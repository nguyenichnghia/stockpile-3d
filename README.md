...
**Toàn bộ roadmap giai đoạn 1–4 đã hoàn tất**: data model + ledger, hai engine lõi (CRP/SLAP), tra cứu + heatmap, picking engine, realtime STOMP, chạy pick-list từng bước trên 3D, quét mã vạch (ADR-0007), reporting dashboard (`/reports`), what-if layout simulation (ADR-0008) và **multi-warehouse** — nhiều kho độc lập trong một CSDL, chọn kho làm việc qua `?wh=` (ADR-0009). Lộ trình & tiến độ: [CHANGELOG](CHANGELOG.md) · [docs/commit-plan.md](docs/commit-plan.md).
