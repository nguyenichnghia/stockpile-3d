...
**Toàn bộ roadmap giai đoạn 1–4 đã hoàn tất** (tag `v0.1.0` → `v1.0.0`): data model + ledger, hai engine lõi (CRP/SLAP), tra cứu + heatmap, picking engine, realtime STOMP, chạy pick-list từng bước trên 3D, quét mã vạch (ADR-0007), reporting dashboard (`/reports`), what-if layout simulation (ADR-0008) và **multi-warehouse** — nhiều kho độc lập trong một CSDL, chọn kho làm việc qua `?wh=` (ADR-0009).

**`v1.1.0`** bồi thêm các slice sau roadmap: enforce quét phía server (`require_scan`), timezone kho cho báo cáo, what-if theo trọng số SLAP, và **chuyển kho** giữa hai kho (cặp OUTBOUND+INBOUND liên kết, ADR-0010). Lộ trình & tiến độ: [CHANGELOG](CHANGELOG.md) · [docs/commit-plan.md](docs/commit-plan.md).
