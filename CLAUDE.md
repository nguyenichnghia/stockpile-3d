...
Roadmap is fully built (post-v1.0.0 slices landed: server-side scan enforcement per ADR-0007's follow-up, per-warehouse report timezone); remaining known slices (no ADR needed unless stated): policy what-if (vary SLAP weights instead of layout), cross-warehouse transfers (needs its own ADR per ADR-0009). Known local quirk: a stale production `.next` makes `next dev` 404 — delete `.next` before `npm run dev` after a build.
