...
> **`bin` là gì?** = một **ô chứa** trong kho (một vị trí cụ thể, vd "Zone A - Kệ 3 - Tầng 2"). Trong code chính là một `location`.
>
> **`from_bin` và `to_bin`** = **"từ ô nào"** và **"tới ô nào"** của một lần di chuyển. Hình dung như khi bạn chuyển đồ: *from* = chỗ lấy đi, *to* = chỗ đặt tới.
> - Hàng **mới vào kho** (INBOUND) thì không có *from* (nó đến từ ngoài) → `from_bin` để trống (—).
> - Hàng **rời kho** (PICK/OUTBOUND) thì không có *to* (nó đi ra ngoài) → `to_bin` để trống.
> - Hàng **dời chỗ trong kho** (RELOCATE) thì có cả hai: *from* = ô cũ, *to* = ô mới.
> Dấu **—** trong bảng nghĩa là "ô đó để trống cho loại này".

| Loại (MovementType) | Nghĩa nghiệp vụ | from_bin (từ ô) | to_bin (tới ô) | Tác động lên `placement` |
|---|---|---|---|---|
...
stateDiagram-v2
    ...
    note right of Placed
        ...
        RELOCATE = đổi bin nhưng vẫn ở trạng thái Placed.
        CRP/SLAP làm việc ở trạng thái này.
    end note
```

> Lưu ý: **RELOCATE** (dời sang bin khác) không đổi *trạng thái* — lô vẫn `Placed`, chỉ đổi vị trí. Vì vậy không vẽ thành mũi tên riêng (tránh lỗi bố cục self-loop của Mermaid).
